...
shared_ptr<pgr_camera::PgrCamera> PgrCameraFactory::getCameraFromSerialNumber(unsigned int serialNumber)
{
  Error error;
  BusManager busMgr;
  PGRGuid cameraGuid;
...
shared_ptr<pgr_camera::PgrCamera> PgrCameraFactory::createGigECamera(PGRGuid cameraGuid,  unsigned int serialNumber)
{
  Error error;
  shared_ptr<pgr_camera::PgrCamera> pgrCamera;
  shared_ptr<GigECamera>  gigeCamera(new GigECamera());
  ...
  shared_ptr<CameraBase> baseCamera = static_pointer_cast<CameraBase>(gigeCamera);

  pgrCamera = shared_ptr<pgr_camera::PgrCamera> (new pgr_camera::PgrCamera(baseCamera,
                                cameraGuid, serialNumber,  FlyCapture2::INTERFACE_GIGE));
  return pgrCamera;
}
